...
# 1) Call saved geotables as geodataframes
...
def json_data(selectedMonth,selectedDay,selectedUrgency):
    ...
    merged =  gpd.GeoDataFrame(pd.concat([gpd.GeoDataFrame(hexgrid1.geometry),gdf_mn_day],axis=1))
...
geosource = GeoJSONDataSource(geojson = json_data(1,1,1))
...
def make_plot(month,day,urgency):    
  ...
  p = figure(title = f"Average number of A{urgency} incidents on {Day_Labels[day-1]}\'s of {Month_Labels[month-1]}", 
